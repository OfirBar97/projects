...
I = imread('ofir_2.bmp'); % Replace with your image file
filename = sprintf('map_of_ones_ofir_2.png');
...
adjustedImage = (double(differenceImage) - minIntensity) *1*(255 / (maxIntensity - minIntensity));
...
% Sauvola threshold
threshold = meanImage .* (1 + k * (stdImage / R - 1));

% Binarize using Sauvola threshold
...
subplot(1,2,2);
imshow(thinned_Img); % Display the binary image
title('Thinned Image');
